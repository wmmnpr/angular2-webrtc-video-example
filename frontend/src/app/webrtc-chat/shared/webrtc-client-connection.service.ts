...
export class WebRTCConnectionService {
  ...
  constructor(
    ...
    this.socket = io.connect('https://rowwithme-webrtc-ice-server.herokuapp.com');
    ...
    this.socket.on(SOCKET_EVENT_PEER_CONNECTED, (data: any) => {
    ...
    this.socket.on(SOCKET_EVENT_PEER_MESSAGE, (data: any) => {
  ...
  private getPeerConnection(id: string): RTCPeerConnection {
    if (this.peerConnections[Number.parseInt(id)]) {
      return this.peerConnections[Number.parseInt(id)];
    }

    const peerConnection = new RTCPeerConnection();
    this.peerConnections[Number.parseInt(id)] = peerConnection;
